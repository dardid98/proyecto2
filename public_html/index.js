...
wrapper.addEventListener("mouseleave", autoSlide);
    ...
    var todasCookies = document.cookie.replace(/(?:(?:^|.*;\s*)activado\s*\=\s*([^;]*).*$)|^.*$/, "$1");
    //alert(todasCookies);
    if(todasCookies=="si"){
        cookies=document.getElementById("cookies").style.display="none";
    }
    botonAcep=document.getElementById("aceptar");
    
    
    //alert(botonRech.value);

    botonAcep.addEventListener("click", ()=>{
        cookies=document.getElementById("cookies").style.display="none";
        document.cookie="activado=si";
    })
